import MergeSort from '../../sorting/merge-sort/MergeSort.js';
...
export default class Knapsack {
  ...
  solveZeroOneKnapsackProblem() {
    ...
    while (itemIndex > 0) {
      ...
      if (
        ...
        if (
          ...
          weightIndex -= prevItem.weight;
        }
      } else if (knapsackMatrix[itemIndex][weightIndex]
        && knapsackMatrix[itemIndex][weightIndex] !== knapsackMatrix[itemIndex - 1][weightIndex]) {
    ...
    if (knapsackMatrix[itemIndex][weightIndex]) {
      ...
      weightIndex -= this.possibleItems[itemIndex];
    }
  }
